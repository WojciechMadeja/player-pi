...
if [ -f /etc/crontab ]; then
    ...
    if [ $exitstatus = 0 ]; then
        echo "Player-pi juz uruchamia sie ze startem systemu"
    else
        echo "@reboot root cd /home/pi/player-pi/ && ./player.sh" | sudo tee -a /etc/crontab
        echo "player-pi bedzie uruchamial sie ze startem systemu"
    fi
else
